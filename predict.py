...
def detect_faces(video_path, detector_cls: Type[VideoFaceDetector], opt):
    ...
    loader = DataLoader(dataset, shuffle=False, num_workers=opt.workers, batch_size=1, collate_fn=simple_collate)
...
def cluster_faces(crops, valid_cluster_size_ratio = 0.20, similarity_threshold = 0.45):
    ...
    if len(faces) == 0:
        print("No faces available for clustering.")
        return {}
...
def get_sorted_identities(identities, discarded_faces, max_identities = 2, num_frames = 8):
    sorted_identities = []
    discarded_faces = []
    ...
    if identities_number > 1:
        ...
        sorted_identities[0][2] = min(len(sorted_identities[0][3]), num_frames)
        available_additional_faces.append(0)
...
def generate_masks(video_path, identities, discarded_faces, num_frames, image_size, num_patches):
    ...
    for identity_index, identity in enumerate(identities):
        ...
        if len(identity_images) > max_faces:
            idx = np.linspace(0, len(identity_images)-1, max_faces).astype(int)
        ...
        if len(identity_images) < max_faces:
            ...
            for i in range(diff):
                identity_images.append(identity_images[i % len(identity_images)])
                identity_size_embeddings.append(identity_size_embeddings[i % len(identity_size_embeddings)])
                images_frames.append(images_frames[i % len(images_frames)])
    ...
    for identity_index in range(len(identities)):
        identity_mask = [True if i >= last_range_end and i < last_range_end + identities[identity_index][2] else False for i in range(num_frames)]
    ...
    for i in range(len(identities)):
        token_end = token_start + identities[i][2] * num_patches
        tokens_per_identity.append((identities[i][0], token_end))
...
def predict(video_path, clustered_faces, config, opt, discarded_faces = None):
    ...
    if opt.extractor_model == 0:  # EfficientNet-B0
        if opt.extractor_weights.lower() == 'imagenet':
            ...
            features_extractor.load_matching_state_dict(
                torch.load(opt.extractor_weights, map_location=torch.device('cpu'))
            )
    ...
    identities, discarded_faces = get_sorted_identities(
        clustered_faces,
        discarded_faces,
        max_identities=2
    )
    videos, size_embeddings, mask, identities_mask, positions, tokens_per_identity = generate_masks(
        ...
        config["model"]["num-patches"]
    )
    b, f, h, w, c = videos.shape
    videos = videos.to(device)    
    size_embeddings = size_embeddings.to(device)
    ...
    with torch.no_grad():
        ...
        pred_score = torch.sigmoid(test_pred).item()
        ...
        return pred_score, identity_attentions, aggregated_attentions, identities, frames_per_identity
...
if __name__ == "__main__":
    ...
    print("Total faces extracted:", len(crops))
    ...
    if pred >= 0.7:
        print("The video is fake ("+str(round(pred*100,2)) + "%)")
    else:
        print("The video is real ("+str(round((1-pred)*100,2)) + "%)")
