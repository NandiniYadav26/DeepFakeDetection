...
class DeepFakesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        relative_path = self.x[index]  # e.g. "original/721.mp4"

        video_folder = relative_path.replace(".mp4", "")
        video_path = os.path.join(self.data_path, video_folder)

        if not os.path.exists(video_path):
            raise Exception("Faces folder not found:", video_path)

        original_video_path = os.path.join(self.video_path, relative_path)
        ...
        for identity_index, identity in enumerate(identities):
            ...
            identity_faces = [
                os.path.join(identity_path, face)
                for face in os.listdir(identity_path)
                if face.lower().endswith((".jpg", ".png", ".jpeg"))
            ]
            ...
            if identity_index == 0 and len(discarded_faces) > 0:
                ...
                frames = [extract_number(p) for p in identity_faces]
                discarded_frames = [extract_number(p) for p in discarded_faces]
            ...
            identity_faces = np.asarray(sorted(identity_faces, key=get_frame_number))
            ...
            if video_area == 0: 
                video_area = self.image_size * self.image_size / 2
            capture.release()
            ...
            if len(identity_images) < max_faces:
                ...
                diff = 0
            if self.enable_identity_attention:
        ...
        if self.multiclass_labels == None:
            return torch.tensor(sequence).float(), torch.tensor(size_embeddings, dtype=torch.int32), torch.tensor(mask).bool(), torch.tensor(identities_mask).bool(), torch.tensor(positions, dtype=torch.int32), self.y[index]
        else:       
            return torch.tensor(sequence).float(), torch.tensor(size_embeddings, dtype=torch.int32), torch.tensor(mask).bool(), torch.tensor(identities_mask).bool(), torch.tensor(positions, dtype=torch.int32), tokens_per_identity, self.y[index], self.multiclass_labels[index], video_id.replace("/", "_")
